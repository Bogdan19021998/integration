...
//        column key by field name
        ...
        List<CustomAttributeDefinition> createdAttributes = ConcurrentUtils.wait(retrieveAllAttributesBySettings().stream().filter(attr -> {
            ...
            return Optional.ofNullable(currentCustomFields.get(fieldId))
                    .map(fieldName -> {
                        result.add(new CustomAttributeDefinition(fieldId, fieldName, attr.getAttributeDataTag(), attr.getId(), attr.getAutoGenerated(), attr.getPosition()));
                        return false;
                    })
                    .orElse(true);
