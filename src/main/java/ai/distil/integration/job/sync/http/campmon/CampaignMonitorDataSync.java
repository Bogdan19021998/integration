...
//        column key by field name
        ...
        List<CustomAttributeDefinition> createdAttributes = ConcurrentUtils.wait(retrieveAllAttributesBySettings().stream()
                .filter(attr -> Optional.ofNullable(currentCustomFields.get(attr.getAttributeDisplayName()))
                        .map(fieldName -> {
                            result.add(new CustomAttributeDefinition(fieldName, attr.getAttributeDisplayName(), attr.getAttributeDataTag(), attr.getId(), attr.getAutoGenerated(), attr.getPosition(), attr.getAttributeType()));
                            return false;
                        })
                    ...
                    return this.httpConnection.getRestService()
                            ...
                            .thenApply(key -> new CustomAttributeDefinition(key, attr.getAttributeDisplayName(), attr.getAttributeDataTag(), attr.getId(), attr.getAutoGenerated(), attr.getPosition(), attr.getAttributeType()));
